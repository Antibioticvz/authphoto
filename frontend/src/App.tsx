...
import './App.css';
import { Camera, CanvasOverlay, CaptureButton, MessageInput, ResultDisplay } from './components';
import { useCamera, useChallenge, useCapture } from './hooks';
import { cryptoService } from './services';
import { recordCanvasVideo, captureCanvasFrame } from './utils';

type AppState = 'idle' | 'requesting' | 'ready' | 'recording' | 'capturing' | 'uploading' | 'complete';
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      const drawComposite = () => {
        ...
        ctx.save();
        ctx.scale(-1, 1);
        ctx.drawImage(videoRef.current, -canvasWidth, 0, canvasWidth, canvasHeight);
        ctx.restore();
      ...
      // Upload to server
      await capturePhoto(photoBlob, challenge.challengeId, hash, message || undefined);
  ...
  const renderContent = () => {
    ...
    return (
      <div className="capture-flow">
        ...
        <div className="camera-container" style={{ position: 'relative', display: 'inline-block' }}>
          <Camera videoRef={videoRef} width={canvasWidth} height={canvasHeight} />
          {challenge && (
            <CanvasOverlay
        ...
        <div className="actions" style={{ marginTop: '20px' }}>
          <CaptureButton
            ...
            isLoading={state === 'recording' || state === 'capturing' || state === 'uploading'}
          />
        </div>
        ...
        {challenge && (
          <div className="challenge-info" style={{ marginTop: '20px', fontSize: '12px', color: '#666' }}>
